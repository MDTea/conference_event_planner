...
export const avSlice = createSlice({
  ...
  reducers: {
    // incrementAvQuantity increments the quantity of a spec item in the state
    incrementAvQuantity: (state, action) => {
        const item = state[action.payload]; // action.payload obj contians the identifier of the item to increment
        if(item){
            item.quantity++;
        }
    },
